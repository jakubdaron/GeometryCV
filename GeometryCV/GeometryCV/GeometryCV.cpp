...
void okreslGeometrie(cv::Mat image, const std::vector<std::vector<cv::Point>>& contours, double area, size_t closestContourIndex)
{
    ...
    for (size_t j = 0; j < contours.size(); ++j)
    {
        ...
        else
        {
            ...
            if (figura == "Triangle" || figura == "Rectangle" || figura == "Square")
            {
...
int main()
{
    ...
    if (image.empty())
    {
        std::cout << "Could not upload the image." << std::endl;
        return -1;
    }
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < contours.size(); ++i)
        {
            if (i == closestContourIndex) // Wyj¹tek dla konturu referencyjnego
            {
                ...
                cv::putText(imageToShow, "Reference", contours[closestContourIndex][0] + cv::Point(-140, 0), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 255), 2);
            }
            else
            {
        ...
        cv::imshow("Original image", imageToShow);

        cv::namedWindow("Image after preprocessing", cv::WINDOW_NORMAL);
        cv::imshow("Image after preprocessing", imgPreToShow);
